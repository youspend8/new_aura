...
<!-- 							</div> -->
							...
							<!-- 비밀번호 찾기 -->
							<div id="pw_find" class="tabcontent" value="pw_find" checked>
								<div class="form-group">
									...
									<form method="POST" class="my-login-validation" novalidate="">
										...
										<div class="custom-control custom-radio">
												<input type="radio" class="custom-control-input" id="defaultGroupExample3"
													name="groupOfDefaultRadios" onclick="Radio_OnOff2('test3')" >
										...
										<form method="POST" class="my-login-validation" novalidate="">
											<div class="custom-control custom-radio">
												<input type="radio" class="custom-control-input"
													...
													onclick="Radio_OnOff2('test4')" checked>
												...
												<div class="form-group" id="test4" style="display: none;">
													...
													<div class="md-form flex-column flex-sm-row" style="display: flex;">
														...
														<label for="identified_email_pw">이메일인증이메일 *</label>
														<button  class="btn btn-primary btn-sm" id="forgot_PWD_duplicate_email" >인증번호</button>
													</div>
														...
														<div style="display: none; color: red; font-size: 15px;"id="eamil_check_false">
															인증 번호가 불 일치 합니다.
														</div>
														
														<div style="display: none; color: red; font-size: 15px;"id="eamil_check_none">
															인증 번호를 입력해주세요합니다.
														</div>
														
														<div style="display: none; color: blue; font-size: 15px;"id="eamil_check_true">
															인증 번호가 일치합니다.
														</div>
														
														<div style="display: none; color: blue; font-size: 15px;"id="eamil_false">
															등록된 이메일이 없습니다.
														</div>
												</div>
											</div>
	...
	//이메일 인증번호 맞는지 확인
		function e_mail() {
			$("#forgot_PWD_duplicate_email").on("click", function() {
				$.ajax({
					url : "/user/emailCheck",
					data : {
		...
		function emailCheck(Check_num) {
			alert(Check_num);
			$('#identified_email_pw').focusout(function() {
				if (Check_num == $("#identified_email_pw").val()) {
					$('#eamil_check_none').css('display', 'none');
					$('#eamil_check_false').css('display', 'none');
					$('#eamil_check_true').css('display', 'inline');
					return true;
				} else if($("#identified_number_idemail_pw") = ""){
					$('#eamil_check_false').css('display', 'none');
					$('#eamil_check_true').css('display', 'none');
					$('#eamil_check_none').css('display', 'inline');
					return false;
				}else if( Check_num == "1"){
					$('#eamil_check_false').css('display', 'none');
					$('#eamil_check_true').css('display', 'none');
					$('#eamil_check_none').css('display', 'none');
					$('#eamil_false').css('display', 'inline');
					return false;
				}else{
					$('#eamil_check_none').css('display', 'none');
					$('#eamil_check_true').css('display', 'none');
					$('#eamil_check_false').css('display', 'inline');
					return false;
				}
			});
		}
